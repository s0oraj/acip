...
import { advancedRangeOperationsAnimation } from './data';

const AdvancedRangeOperationsVisualizer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      timer = setTimeout(() => {
        if (step < advancedRangeOperationsAnimation.steps.length - 1) {
          setStep(s => s + 1);
        } else {
          setIsPlaying(false);
        }
  ...
  const { visualizationData } = advancedRangeOperationsAnimation.steps[step];

  return (
    <div className="p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold mb-2">{advancedRangeOperationsAnimation.title}</h2>
        <p className="text-gray-600">{advancedRangeOperationsAnimation.description}</p>
      </div>
      
      <div className="mb-4">
        <div className="flex space-x-2">
          {advancedRangeOperationsAnimation.steps.map((s, index) => (
            <button
              key={index}
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm mb-6">
        <h3 className="text-xl font-semibold mb-4">{advancedRangeOperationsAnimation.steps[step].title}</h3>
        <p className="text-gray-600 mb-4">{advancedRangeOperationsAnimation.steps[step].description}</p>
      ...
      <div className="flex justify-center gap-4">
        ...
        <button
          onClick={() => setStep(s => Math.min(advancedRangeOperationsAnimation.steps.length - 1, s + 1))}
          className="w-12 h-12 rounded-full flex items-center justify-center bg-gray-200 hover:bg-gray-300 text-gray-700 transition-all duration-300"
        >
...
export default AdvancedRangeOperationsVisualizer;
