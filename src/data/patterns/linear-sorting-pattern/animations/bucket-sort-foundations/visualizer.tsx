...
import { bucketSortAnimation } from './data';
...
const BucketSortVisualizer: React.FC = () => {
  ...
  useEffect(() => {
    setCurrentArray(bucketSortAnimation.steps[step].array);
  }, [step]);

  useEffect(() => {
    let timer: NodeJS.Timeout;
    if (isPlaying && step < bucketSortAnimation.steps.length - 1) {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold mb-2">{bucketSortAnimation.steps[step].title}</h2>
        <p className="text-gray-600">{bucketSortAnimation.steps[step].description}</p>
      </div>
      ...
      <div className="bg-gray-800 p-3 rounded-lg mb-6">
        <pre className="text-sm text-white overflow-x-auto">
          <code>{bucketSortAnimation.steps[step].code}</code>
        </pre>
      </div>
      ...
      <div className="flex justify-center gap-4">
        ...
        <button
          onClick={() => setStep(s => Math.min(bucketSortAnimation.steps.length - 1, s + 1))}
          className="w-12 h-12 rounded-full flex items-center justify-center bg-gray-200 hover:bg-gray-300 text-gray-700 transition-all duration-300"
        >
