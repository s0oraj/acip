...
import { patterns, specialRangeOperationsAnimation } from './data';

const Visualizer: React.FC = () => {
  const [activePattern, setActivePattern] = useState('rangeMedianQuery');
  const [step, setStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);

  useEffect(() => {
    let timer: NodeJS.Timeout;
    if (isPlaying && step < specialRangeOperationsAnimation.steps[0].phases.length - 1) {
  ...
  const renderVisualization = () => {
    const currentStep = specialRangeOperationsAnimation.steps[0];
    ...
    return (
      <div className="space-y-4">
        ...
        {currentPhase.median !== undefined && (
          <div className="bg-green-100 p-2 rounded">
            Median: {currentPhase.median}
          </div>
        )}
      </div>
    );
    ...
    <div className="p-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm mb-6">
        <h3 className="text-lg font-semibold mb-4">
          {specialRangeOperationsAnimation.steps[0].title}
        </h3>
        {renderVisualization()}
      </div>
      ...
      <div className="mt-4">
        <div className="flex justify-between text-sm text-gray-500 mb-1">
          ...
          <span>of {specialRangeOperationsAnimation.steps[0].phases.length}</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            className="bg-blue-500 h-2 rounded-full transition-all duration-300"
            style={{ width: `${((step + 1) / specialRangeOperationsAnimation.steps[0].phases.length) * 100}%` }}
          />
        </div>
      </div>
    </div>
