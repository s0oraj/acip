...
import { patterns, treeSerialization } from './data';
...
const Visualizer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isPlaying && step < treeSerialization.steps[activePattern].phases.length - 1) {
  ...
  const renderVisualization = () => {
    const currentStep = treeSerialization.steps[activePattern];
    ...
    <div className="p-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm mb-6">
        <h3 className="text-lg font-semibold mb-4">
          {treeSerialization.steps[activePattern].title}
        </h3>
        {renderVisualization()}
      </div>
      ...
      <div className="bg-gray-100 p-4 rounded-xl mb-4">
        <pre className="text-sm overflow-x-auto">
          <code>
            {treeSerialization.steps[activePattern].phases[step].description}
          </code>
        </pre>
      </div>
      ...
      <div className="mt-4">
        <div className="flex justify-between text-sm text-gray-500 mb-1">
          ...
          <span>of {treeSerialization.steps[activePattern].phases.length}</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            className="bg-blue-500 h-2 rounded-full transition-all duration-300"
            style={{ width: `${((step + 1) / treeSerialization.steps[activePattern].phases.length) * 100}%` }}
          />
        </div>
      </div>
    </div>
