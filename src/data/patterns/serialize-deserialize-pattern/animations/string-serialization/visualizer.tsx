...
import { patterns, stringSerialization } from './data';
...
const Visualizer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isPlaying && step < stringSerialization.steps[activePattern].phases.length - 1) {
  ...
  const renderVisualization = () => {
    const currentStep = stringSerialization.steps[activePattern];
    ...
    return (
      <div className="space-y-4">
        ...
        {currentStep.delimiter && (
          <div className="bg-green-100 p-2 rounded">
            Delimiter: {currentStep.delimiter}
          </div>
        )}
    ...
    <div className="p-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm mb-6">
        <h3 className="text-lg font-semibold mb-4">
          {stringSerialization.steps[activePattern].title}
        </h3>
        {renderVisualization()}
      </div>
      ...
      <div className="bg-gray-100 p-4 rounded-xl mb-4">
        <pre className="text-sm overflow-x-auto">
          <code>
            {stringSerialization.steps[activePattern].phases[step].description}
          </code>
        </pre>
      </div>
      ...
      <div className="mt-4">
        <div className="flex justify-between text-sm text-gray-500 mb-1">
          ...
          <span>of {stringSerialization.steps[activePattern].phases.length}</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            className="bg-blue-500 h-2 rounded-full transition-all duration-300"
            style={{ width: `${((step + 1) / stringSerialization.steps[activePattern].phases.length) * 100}%` }}
          />
        </div>
      </div>
    </div>
